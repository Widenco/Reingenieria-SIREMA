...
import {
  ...
  crearTipoModalidad,
  actualizarTipoModalidad,
  cambiarEstadoTipoModalidad,
} from '../../api/endpoints/tiposModalidad.js';
...
export function TiposModalidadPage() {
  ...
  const abrirEditar = (tipo) => {
    ...
    setDescripcion(tipo.DescripcionDeTipo);
    setModalAbierto(true);
  };
  ...
  const guardar = async (e) => {
    ...
    try {
      setGuardando(true);
      if (tipoEditando) {
        await actualizarTipoModalidad(tipoEditando.Id, { descripcion: descripcion.trim() });
        setNotificacion({
          tipo: 'exito',
          titulo: '¡Guardado!',
          mensaje: 'El Tipo de Modalidad ha sido actualizado exitosamente.',
        });
      } else {
        await crearTipoModalidad({ descripcion: descripcion.trim() });
        setNotificacion({
          tipo: 'exito',
          titulo: '¡Guardado!',
          mensaje: 'El Tipo de Modalidad ha sido creado exitosamente.',
        });
      }
      cerrarModal();
      cargar();
      ...
      setNotificacion({
        tipo: 'error',
        titulo: 'Error',
        mensaje: err?.response?.data?.error || 'No fue posible guardar el tipo de modalidad.',
      });
    } finally {
      setGuardando(false);
    }
  ...
  const ejecutarCambioEstado = async () => {
    ...
    try {
      ...
      await cambiarEstadoTipoModalidad(confirmacion.tipo.Id);
      setConfirmacion(null);
      setNotificacion({
        tipo: 'exito',
        titulo: '¡Guardado!',
  ...
  return (
    <main className={styles.page}>
      <header className={styles.header}>
        ...
        <h1>Tipos Modalidad CNU</h1>
        <span>Tipos de modalidad CNU registrados en el sistema.</span>
      </header>
      ...
      {modalAbierto && (
        <FormularioModal
          titulo={tipoEditando ? 'Editar Tipo Modalidad' : 'Nuevo Tipo Modalidad'}
          label="Descripción"
          valor={descripcion}
