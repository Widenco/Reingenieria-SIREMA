...
import { crearSchema, actualizarSchema } from '../validators/turno.schema.js';
...
export async function listarPorModalidad(req, res, next) {
  try {
    res.json(await service.porModalidad(Number(req.params.modalidadId)));
  } catch (err) {
    next(err);
  }
}
