import * as service from '../services/anioLectivo.service.js';
import { crearSchema, actualizarSchema } from '../validators/anioLectivo.schema.js';
...
export async function obtener(req, res, next) {
  try {
    ...
    if (!item) return res.status(404).json({ error: 'Año lectivo no encontrado' });
    res.json(item);
  } catch (err) {
    next(err);
  }
