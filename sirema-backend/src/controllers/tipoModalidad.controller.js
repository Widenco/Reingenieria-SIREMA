import * as service from '../services/tipoModalidad.service.js';
import { crearSchema, actualizarSchema } from '../validators/tipoModalidad.schema.js';
...
export async function obtener(req, res, next) {
  try {
    ...
    if (!item) return res.status(404).json({ error: 'Tipo de modalidad no encontrado' });
    res.json(item);
  } catch (err) {
    next(err);
  }
