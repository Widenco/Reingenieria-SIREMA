import * as service from '../services/anioCarrera.service.js';
import { crearSchema, actualizarSchema } from '../validators/anioCarrera.schema.js';
...
export async function obtener(req, res, next) {
  try {
    ...
    if (!item) return res.status(404).json({ error: 'Año de carrera no encontrado' });
    res.json(item);
  } catch (err) {
    next(err);
  }
