import * as service from '../services/areaConocimiento.service.js';
import { crearSchema, actualizarSchema } from '../validators/areaConocimiento.schema.js';
...
export async function obtener(req, res, next) {
  try {
    ...
    if (!item) return res.status(404).json({ error: 'Área de conocimiento no encontrada' });
    res.json(item);
  } catch (err) {
    next(err);
  }
