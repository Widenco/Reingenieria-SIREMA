import * as service from '../services/modalidad.service.js';
import { crearSchema, actualizarSchema } from '../validators/modalidad.schema.js';
...
export async function obtener(req, res, next) {
  try {
    ...
    if (!item) return res.status(404).json({ error: 'Modalidad no encontrado' });
    res.json(item);
  } catch (err) {
    next(err);
  }
