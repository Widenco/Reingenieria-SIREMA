import * as service from '../services/tipoIngreso.service.js';
import { crearSchema, actualizarSchema } from '../validators/tipoIngreso.schema.js';
...
export async function obtener(req, res, next) {
  try {
    ...
    if (!item) return res.status(404).json({ error: 'Tipo de ingreso no encontrado' });
    res.json(item);
  } catch (err) {
    next(err);
  }
