import * as service from '../services/semestre.service.js';
import { crearSchema, actualizarSchema } from '../validators/semestre.schema.js';
...
export async function obtener(req, res, next) {
  try {
    ...
    if (!item) return res.status(404).json({ error: 'Semestre no encontrado' });
    res.json(item);
  } catch (err) {
    next(err);
  }
